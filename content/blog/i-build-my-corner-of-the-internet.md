...
### Inline or external
CSS used to be inlined right into every page: it was only 5.7KB, and skipping the extra round-trip before the first paint felt worth it. But the stylesheet grew past 28KB since, and more importantly, the site turned into that SPA-style router I described above: internal navigation swaps the page without a full reload, reusing whatever's already sitting in cache. Inlining meant paying for the same CSS again on every single page instead of once, ever. So it moved back out to an external, fingerprinted file, cached forever (`immutable`, a year-long `max-age`) and fetched once per visitor, then reused across the whole site.

The one thing that mattered getting right on the way out: preloading it. A `<link rel="preload" as="style" crossorigin>` in the response headers kicks the fetch off before the browser even reaches the stylesheet `<link>` in `<head>`, so even a first-ever visit doesn't pay an extra round-trip for it, and every visit after pays even less.

Icons work the same way, and always have: I don't inline them into the page, I bundle them into one external SVG sprite: each icon becomes a `<symbol>`, and the page keeps only a short `<use href="…">` reference to it. The sprite loads once, gets cached, and serves every icon from then on. Same story now with CSS and JS: anything blocking-but-static lives in its own fingerprinted, forever-cached file, fetched once and reused everywhere instead of shipped fresh with every page.
